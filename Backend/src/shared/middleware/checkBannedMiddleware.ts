...
export const checkBanned = (req: Request, res: Response, next: NextFunction) => {
    if (req.user && req.user.isBanned) {
        return res.status(StatusCodes.FORBIDDEN).json({
            success: false,
            message: `Your account has been restricted/banned. Reason: ${req.user.banReason || 'Violation of community guidelines'}. You are restricted from posting, commenting, or writing content.`
        });
    }
    next();
};
